...
class TensorflowAddonsNeeded(Exception):
    """Exception raised when Tensorflow Addons are needed but not installed."""
...
class TFGraphBuilder:
    ...
    def supports_auto_file_name(self):
        """Returns True if the model supports automatic file name generation.
    ...
    def get_model_filename(self):
        ...
        ------
        Exception
            If the method is not implemented in the child classes.
        """
    ...
    def get_build_params_with_defaults(self) -> Dict:
        ...
        # TODO: shouldn't we iterate over key, value pairs?
    ...
    def get_build_param(self, build_param: str) -> Any:
    ...
    def get_model_build_params(self) -> Dict:
    ...
    def get_model_build_param_explanations(self) -> Dict:
...
class GenericClassifierTFGraphBuilder(TFGraphBuilder):
    ...
    This created the graph and saved it to /content/gc_graph/gcl.302.2.pb
    We can use it in the GenericClassifierApproach annotator by setting the modelFile parameter
    ...
    def get_model_filename(self) -> str:
        ...
        str
            The model file name.
        """
        return f'gcl.{self.get_build_param("input_dim")}.{self.get_build_param("output_dim")}.pb'
    ...
    def get_model_build_param_explanations(self) -> Dict[str, str]:
        ...
        return {
            ...
            "output_act": "Output activation function: softmax, sigmoid or linear.",
            "loss_func": "Loss function: cross_entropy, mean_squares or hinge"
        }

    def build(self, model_location, model_filename) -> None:
        """
        ...
        ----------
        model_location : str
            The location where the model will be saved.
        model_filename : str
...
class LogRegClassifierTFGraphBuilder(GenericClassifierTFGraphBuilder):
    def get_model_build_params(self) -> Dict[str, Any]:
        """
        Returns the build parameters for the model.
...
class SVMClassifierTFGraphBuilder(GenericClassifierTFGraphBuilder):
    def get_model_build_params(self) -> Dict[str, Any]:
        """
        Returns the build parameters for the model.
...
class FewShotClassifierTFGraphBuilder(LogRegClassifierTFGraphBuilder):
    pass
...
class AssertionTFGraphBuilder(TFGraphBuilder):
    ...
    def get_model_filename(self) -> str:
        ...
        str
            The model file name.
        """
        n_hidden = self.get_build_param("n_hidden")
        feat_size = self.get_build_param("feat_size")
        n_classes = self.get_build_param("n_classes")
        return f"blstm_{n_hidden}_32_30_{feat_size}_{n_classes}.pb"
...
class NerTFGraphBuilder(TFGraphBuilder):
    ...
    Builds a graph with embeddings_dim=200, nchars=83, ntags=12 for medical NER.
    >>> from sparknlp_jsl.training import tf_graph
    >>> from sparknlp_jsl.base import *
    >>> from sparknlp.annotator import *
    >>> from sparknlp_jsl.annotator import *
    ...
    ...                build_params={"embeddings_dim": 200, "nchars": 83,"ntags": 12,"is_medical": 1},
...
class RelationExtractionTFGraphBuilder(GenericClassifierTFGraphBuilder):
    ...
    Creates the graph for RelationExtractionApproach
    >>> from sparknlp_jsl.training import tf_graph
    >>> from sparknlp_jsl.base import *
    >>> from sparknlp.annotator import *
    >>> from sparknlp_jsl.annotator import *
    ...
    def get_model_filename(self) -> str:
        ...
        str
            The model file name.
        """
        return "rel_e.in{}.out{}.pb".format(
            self.get_build_param("input_dim"),
            self.get_build_param("output_dim"),
        )

class FewShotAssertionTFGraphBuilder(GenericClassifierTFGraphBuilder):
    """
    Class to build the the TF graphs for FewShotAssertionClassifierApproach

    Examples
    --------

    Creates the graph for FewShotAssertionClassifierApproach
    >>> from sparknlp_jsl.training import tf_graph
    >>> from sparknlp_jsl.base import *
    >>> from sparknlp.annotator import *
    >>> from sparknlp_jsl.annotator import *
    >>> from sparknlp_jsl.annotator import *
    >>> tf_graph.build("fewshot_assertion",
    ...                build_params={"input_dim": 768,
    ...                              "output_dim": 3,
    ...                              "batch_norm": 1,
    ...                              "hidden_layers": [300, 200],
    ...                              "hidden_act": "relu",
    ...                              "hidden_act_l2": 1},
    ...                 model_location=".",
    ...                 model_filename="few_shot_assertion")
    >>> few_shot_assertion_approach = FewShotAssertionClassifierApproach() \\
    ...    .setLabelColumn("label") \\
    ...
    def get_model_filename(self) -> str:
        ...
        str
            The model file name.
        """
        return "fewshot_assertion.in{}.out{}.pb".format(
            self.get_build_param("input_dim"),
            self.get_build_param("output_dim"),
        )
...
class TFGraphBuilderFactory:
    ...
    generic_classifier, assertion_dl, relation_extraction annotators in Healthcare NLP.
    """
    ...
    __model_builders = {
        ...
        "logreg_classifier": LogRegClassifierTFGraphBuilder,
        "svm_classifier": SVMClassifierTFGraphBuilder,
        "fewshot_classifier": FewShotClassifierTFGraphBuilder,
        "fewshot_assertion": FewShotAssertionTFGraphBuilder,
    }
    ...
    def build(model_name, build_params, model_location, model_filename="auto"):
        ...
        ----------
        model_name: str
            The name of the tf model that you want to build.
            Available models are ner_dl, generic_classifier, assertion_dl and relation_extraction.
        build_params: dict
            Configuration params to build the tf graph for the specific model.
        model_location: str
            Path where the model will be saved.
        model_filename: str
            Name of the .rb file. If "auto" (default), the filename will be generated.

        Raises
        ------
        Exception
            If the `model_name` is not supported.
            If `model_filename` is "auto" but `model_name` doesn't support automatic filename generation.
        ModuleNotFoundError
            If tensorflow 1.15 or 2.xx is not installed.
        WrongTFVersion
            If the tf version is not 1.15 or 2.xx.
        TensorflowAddonsNeeded
            If tensorflow addons is not installed.
        ...
        try:
            ...
            print(tf.version)
            raise Exception(
                "Tensorflow 2.xx or 1.15 is required to build model graphs."
            )

        except ModuleNotFoundError:
            raise Exception(
                "You need to install Tensorflow 2.xx or 1.15 to be able to build model graphs"
            )

        except TensorflowAddonsNeeded:
            raise Exception(
                "You need to install tensorflow_addons to be able to generate graphs in Tensorflow 2.x"
            )
    ...
    def print_model_params(model_name):
        """
        Method that return the params allowed for the tf model.
        This method return the params with the description for every param.

        Parameters
        ----------
        model_name: str
            The name of the tf model name.
            Available models are: ner_dl, generic_classifier, assertion_dl and relation_extraction
