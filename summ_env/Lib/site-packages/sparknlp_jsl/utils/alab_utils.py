...
def get_doc_assembler(doc_wise_annot=False) -> DocumentAssembler:
    global document_assembler

    output_col = "sentence" if doc_wise_annot else "document"
    document_assembler = DocumentAssembler().setInputCol("text").setOutputCol(output_col)
...
def get_sentence_pipeline(spark, doc_wise_annot=False):
    """Generates a LightPipeline with a document assembler and sentence detector
        :param spark: Spark session with spark-nlp-jsl jar
        :type spark: SparkSession
        :param doc_wise_annot: drops sentence detector if True
        :type doc_wise_annot: bool
        :return: LightPipeline with a document assembler and sentence detector
        :rtype: LightPipeline
        """
    
    _stages = [get_doc_assembler(doc_wise_annot)]
    if not doc_wise_annot:
        _stages.append(get_sent_detector())
...
def get_ner_df(input_json_path, ground_truth=False, assertion_labels=None):
    ...
    # strip out the text from the entities and update the start and end positions
    ann_results = strip_json_entities(ann_results)
...
def get_ner_sentence_borders(spark, input_json_path, doc_wise_annot=False, ground_truth=False, assertion_labels=None):
    """Get sentence borders for each task
    :param spark: Spark session with spark-nlp-jsl jar
    :type spark: SparkSession
    :param input_json_path: path to Annotation Lab JSON export
    :type input_json_path: str
    :param doc_wise_annot: returns the dataframe with document wise annotations
    :type doc_wise_annot: bool
    :param ground_truth: set to True to select ground truth completions, False to select latest completions,
    defaults to False
    :type ground_truth: bool
    ...
    df = get_sentence_pipeline(spark=spark, doc_wise_annot=doc_wise_annot).transform(task_sent_spark_df)
...
def get_single_task_conll(output, pos_pipeline, token_pipeline, ground_truth=False, excluded_labels=None,
    ...
    if len(output['completions']) > 0 and title.strip() not in excluded_task_titles \
        ...
        for _, p in enumerate(parsed):
            ...
            if tag_dict.get((p[2], p[1])):
                  conll_lines.append("{} {} {} {}\n".format(p[1], p[4], p[4], tag_dict.get((p[2], p[1]))))
   
            else:
                  conll_lines.append("{} {} {} {}\n".format(p[1], p[4], p[4], "O"))
...
def read_conll(spark, filepath):
        ...
        Returns:
          _type_: Spark DataFrame

        """
        global _ROW_SCHEMA, _DF_SCHEMA
        ...
        with open(filepath, 'r') as conll:
            ...
            for line in conll:
                ...
                if line is '\n':
                    ...
                    if end != 0:
                      sentence = " ".join([t.result for t in token_rows])
                      sentence_samples.append({
                                'text': sentence,
                                'document_id': doc_indx,
                                'document': [Row(
                                ...
                                'sentence_indx': str(sentence_count),
                                'pos': pos_tag_rows,
                                'token': token_rows,
                                'label': label_rows
                            })
                ...
                if line.startswith('-DOCSTART-'):
                    doc_start, _, _doc_indx, _ = line.strip().split()
                    if _doc_indx.strip('-').isnumeric():
                      doc_indx = _doc_indx.strip('-')
                    else:
                      doc_indx += 1
                    continue
        ...
        return spark.createDataFrame(sentence_samples, _DF_SCHEMA)

def strip_json_entities(exported_json):
    """iterate through entities and strip out the text
    and update start and end positions in the json file from annotation lab

    Args:
        exported_json (dict): exported json from annotation lab

    Returns:
        exported_json (dict): updated json
    """

    # iterate through the tasks
    for task in exported_json:

        # iterate through the completions of each task
        for completion in task["completions"]:

            # iterate through the results of each completion
            for result in completion["result"]:

                # if the result is a NER or Assertion entity type
                if "value" in result.keys() and result["type"] == "labels":
                    temp_text = result["value"]["text"].strip()

                    # if the text has changed
                    if temp_text != result["value"]["text"]:

                        # update the start position
                        result["value"]["start"] = result["value"]["start"] + result["value"]["text"].find(temp_text)
                        
                        # update the end position
                        result["value"]["end"] = result["value"]["start"] + len(temp_text)

                        # update the text
                        result["value"]["text"] = temp_text
