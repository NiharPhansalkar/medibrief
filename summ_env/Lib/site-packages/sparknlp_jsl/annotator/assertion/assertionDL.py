...
class AssertionDLApproach(AnnotatorApproachInternal, HasEngine, HandleExceptionParams):
    ...
    The training data should have annotations columns of type ``DOCUMENT``, ``CHUNK``,
    ``WORD_EMBEDDINGS``, the ``label`` column (the assertion status that you want to
    predict), the ``start`` (the start index for the term that has the assertion status),
    ...
    graphFolder
        Folder path that contain external graph files. The path can a local file path,
    ...
    validationSplit
        The proportion of training dataset to be used as validation set, meaning
        that the model will be validated against this dataset on each Epoch and will not
        be used for training. The value should be between 0.0 and 1.0
    ...
    def setConfigProtoBytes(self, conf):
        ...
        ----------
        conf : bytes
            The byte array contaiing the ConfigProto from tensorflow.
        """
    ...
    def setDropout(self, rate: float):
        ...
        ----------
        rate : float
            Dropout rate at the output of each layer
        """
    ...
    def setMaxSentLen(self, length: int):
        ...
        ----------
        length : int
            The new value for maximum sentence length.
        """
    ...
    def setTestDataset(self, path: str, read_as=ReadAs.SPARK, options=None):
        ...
        ----------
            path : srt
                Path to test dataset. If set used to calculate statistic on
                it during training.
            read_as : str
                Read as format. Default is "SPARK" (ReadAs.SPARK).
            options : dict
                Options for reading. Default is None.
        """
        if options is None:
    ...
    def setIncludeConfidence(self, value: bool):
        ...
        ----------
        value : bool
            Whether to use confidence scores in annotation metadata or not.
        """
    ...
    def __init__(self):
        ...
        self._setDefault(label="label", batchSize=64, epochs=5, learningRate=0.0012, dropout=0.05, maxSentLen=250,
                         includeConfidence=True, scopeWindow=[9, 15])


class AssertionDLModel(AnnotatorModelInternal, HasStorageRef,HandleExceptionParams):
    ...
    entityAssertionCaseSensitive = Param(Params._dummy(), "entityAssertionCaseSensitive", "Determines whether entity labels and assertion labels are case sensitive",
                                         TypeConverters.toBoolean)
    ...
    def __init__(self, classname="com.johnsnowlabs.nlp.annotators.assertion.dl.AssertionDLModel", java_model=None):
        ...
        self._setDefault(
            scopeWindow=[9, 15],
            entityAssertionCaseSensitive=False
        )
    ...
    def setEntityAssertionCaseSensitive(self, value: bool):
        """Sets the case sensitivity of entities and assertion labels
        
        Parameters
        ----------
        value : bool
            whether entities and assertion labels are case sensitive
        """
        return self._set(entityAssertionCaseSensitive=value)

    def setEntityAssertion(self, assertionEntities):
        ...
        Parameters
        ----------
        assertionEntities : dict[str, list[str]]
            List of assertion labels allowed for a given entity

        """

        self._call_java("setEntityAssertion", assertionEntities)
        return self

    def getEntityAssertion(self):
        """
        Return the lists of assertion labels allowed for a given entity
        """

        assertion_entities = self._call_java("getEntityAssertionStr")
        assertion_entities_dict = {}
        for t in assertion_entities.split("\n"):
            parts = t.split(":")
            if len(parts) == 2:
                entity, labels = parts
                labels = labels.split(",")
                assertion_entities_dict[entity] = labels

        return assertion_entities_dict

    def setReplaceLabels(self, replaceLabels):
        ...
        Parameters
        ----------
        replaceLabels : dict[str, str]
            The assertion labels to be replaced for the specified new labels.
        """
        
        self._call_java("setReplaceLabels", replaceLabels)
        return self

    def getReplaceLabels(self):
        """Gets the assertion labels to be replaced for the specified new labels.
        """

        replace_labels = self._call_java("getReplaceLabelsStr")
        replace_labels_dict = {}
        for t in replace_labels.split("\n"):
            parts = t.split(":")
            if len(parts) == 2:
                old_label, new_label = parts
                replace_labels_dict[old_label] = new_label

        return replace_labels_dict
    ...
    @staticmethod
    def pretrained(name="assertion_jsl_augmented", lang="en", remote_loc="clinical/models"):
        """Downloads and loads a pretrained model.

        Parameters
        ----------
        name : str, optional
            Name of the pretrained model, by default "assertion_jsl_augmented"
        lang : str, optional
