...
class MergeFeatureParams:
    ...
    def setFalsePositives(self, value):
        """List of entity pairs that are false positives. If a third value is defined, the pair will be replaced by that value.
        >>>
        ... .setFalsePositives([["is trying", "CHANGE", "WOW"],["beautiful thing", "BLOCK", ""]])
        ...
        self._call_java("setFalsePositives", value)
        return self

    def setReplaceDict(self, replaceDict):
        ...
        self._call_java("setReplaceDict", replaceDict)
        return self
    ...
    def getReplaceDict(self):
        ...
        rep = self._call_java("getReplaceDictStr")
        rep_dict = parse_str_to_dict(rep)
        return rep_dict

class ChunkMergeApproach(AnnotatorApproachInternal, MergeCommonParams, MergePrioritizationParams, MergeResourceParams, FilteringParams,HandleExceptionParams):
    """
    ...
    entitiesConfidence :  dict[str, float]
        Pairs (entity,confidenceThreshold) to filter
    ...
    def setEntitiesConfidence(self, entitiesConfidence:dict):
        ...
        entities_confidence : dict[str, float]
            ...
            which have confidence lower than the confidence threshold.
        """
        self._call_java("setEntitiesConfidence", entitiesConfidence)
        return self
...
class ChunkMergeModel(AnnotatorModelInternal, MergeCommonParams, MergePrioritizationParams, FilteringParams, MergeFeatureParams,HandleExceptionParams):
    """
    ...
    filterValue: str
        If equal to "entity", use the ner label to filter. If set to "result",
        use the `result` attribute of the annotation to filter.
    falsePositives : List[List[str]]
        List of entity pairs that are false positives. If a third value is defined,
        the pair will be replaced by that value.
    replaceDict : dict[str, str]
        dictionary with regular expression patterns that match some protected entity
    """
