...
class TextToDocumentsColumns:
    ...
    def toDocumentsColumnsWithId(self, df: DataFrame, id_column: str):
        """Converts a DataFrame with columns of string texts
        into a DataFrame with columns of Document annotations having id in the metadata.
        """
        ...
        if match:
            version = int("".join(match.group(0).split(".")))
            if version >= 330:
                return DataFrame(self.instance.toDocumentsColumnsWithId(df._jdf, id_column), self.spark._getActiveSessionOrCreate())
            else:
                return DataFrame(self.instance.toDocumentsColumnsWithId(df._jdf, id_column), self.spark._wrapped)
        else:
            try:
                return DataFrame(self.instance.toDocumentsColumnsWithId(df._jdf, id_column), self.spark._getActiveSessionOrCreate())
            except:
