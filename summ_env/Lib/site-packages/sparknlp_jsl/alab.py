...
class AnnotationLab():
    ...
    def get_conll_data(
        ...
        :type save_dir: str
        :param ground_truth: set to True to select ground truth completions, False to select latest completions, defaults to False
        :type ground_truth: bool
        :param excluded_labels: labels to exclude from CoNLL; these are all assertion labels and irrelevant NER labels, defaults to None
    ...
    def get_conll_data_from_visualner(
        self,
        input_json_path: str,
        output_name: str = "visualner",
        save_dir: str = "exported_conll") -> IO:
        """Generates a CoNLL file from an NLP Lab JSON export.
        ...
        bulk_conll_lines = []
        bulk_conll_lines.append("-DOCSTART- -X- -X- O \n\n")
        ...
        for item in json_outputs:
            ...
            for tokens in ocr_texts:
                ...
                for i, token in enumerate(tokens):
                    ...
                    # Check if the token text is present in any of the text fields within tokens
                    ...
                    # Adjust label prefix based on whether it's O or not
                    label_prefix = "NN NN" if label == "O" else "NNP NNP"

                    if label != "O":
                        # Check if the previous label is the same as the current one
                    ...
                    bulk_conll_lines.append(conll_line)

                    prev_label = label

                bulk_conll_lines.append("\n")
        try:
            os.mkdir(save_dir)
        except:
            pass
                ...
                for _, i in enumerate(output['completions'][gt_index]['result']):
                    ...
                    if i['type'] == 'labels':
                        ...
                        ner_dicts.append((doc_id, sent, i['value']['text'], assertion_label, left_ix, right_ix,
                                          i['value']['start'], i['value']['end'], begin, end-1))
    ...
    def review_annotations(self, spark: SparkSession, conll_path: str, ner_model_path: str):
        ...
        :param spark: spark session
        :type spark: SparkSession
        :param conll_path: path to conll file
        :type conll_path: str
        :param ner_model_path: path to ner model
        :type ner_model_path: str
        
        :return: tuple containing chunk_based_comparison_df (DataFrame), token_based_comparision_df (DataFrame), chunk_frequency_df (DataFrame), metrics_result (Dict)
        :rtype: tuple
