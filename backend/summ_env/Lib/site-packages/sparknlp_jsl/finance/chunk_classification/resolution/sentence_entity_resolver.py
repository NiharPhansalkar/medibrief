...
class SentenceEntityResolverApproach(A):
    """Contains all the parameters and methods to train a SentenceEntityResolverModel.
     ...
     The model transforms a dataset with Input Annotation type SENTENCE_EMBEDDINGS,
     coming from e.g. [BertSentenceEmbeddings](/docs/en/transformers#bertsentenceembeddings)
     ...
     pretrainedModelPath
          Path to an already trained SentenceEntityResolverModel, which is used as a
          starting point for training the new model. The path can be a local file path,
          a distributed file path (HDFS, DBFS), or a cloud storage (S3).
     overrideExistingCodes
          Whether to override the existing codes with new data while continue the training
          from a pretrained model. Default value is false(keep all the codes).
     ...
     overrideExistingCodes
          Whether to override the codes present in a pretrained model with new codes
          when the training process begins with a pretrained model
     dropCodesList
          A list of codes in a pretrained model that will be omitted when the training
          process begins with a pretrained model
    ...
    def __init__(self):
        AnnotatorApproachInternal.__init__(self,
                                   classname="com.johnsnowlabs.finance.chunk_classification.resolution.SentenceEntityResolverApproach")
...
class SentenceEntityResolverModel(M):
    """Thius class contains all the parameters and methods to train a SentenceEntityResolverModel.
       ...
       """
    def __init__(self,
                 classname="com.johnsnowlabs.finance.chunk_classification.resolution.SentenceEntityResolverModel",
                 java_model=None):
    ...
    @staticmethod
    def pretrained(name="finel_edgar_company_name", lang="en", remote_loc="finance/models"):
        ...
        Returns:
            SentenceEntityResolverModel: A pre-trained SentenceEntityResolverModel.
        """
