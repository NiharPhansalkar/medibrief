...
class GenericClassifierModel(BaseTFModel):
    ...
    def build(self, params):
        ...
        if "output_act" not in params:
            params["output_act"] = "softmax"

        if "loss_func" not in params:
            params["loss_func"] = "cross_entropy"
        ...
        with tf.Session() as session:
            ...
            if ("batch_norm" in params) and params["batch_norm"]:
                normalization_layer = keras.layers.BatchNormalization()(output_layer)
            else:
            ...
            if params["output_act"] == "sigmoid":
                outputs = tf.sigmoid(output_layer,name="outputs")
            elif params["output_act"] == "softmax":
                outputs = tf.math.softmax(output_layer, name="outputs")
            elif params["output_act"] == "linear":
                outputs = tf.identity(output_layer, name="outputs")
            else:
                raise Exception("Activation function " + params["output_act"] + " not supported.")

            if params["loss_func"] == "hinge":
                loss = tf.identity(
                    tf.keras.losses.CategoricalHinge(name="loss_f")(targets, output_layer),
                    name="loss")
            elif params["loss_func"] == "cross_entropy":
                if params["output_act"] == "softmax":
                    loss = tf.reduce_mean(
                        ...
                        name='loss')
                elif params["output_act"] == "sigmoid":
                    loss = tf.reduce_mean(
                    ...
                    raise Exception("Activation function " + params["output_act"] + " not supported.")
            elif params["loss_func"] == "mean_squares":
                #default
                loss = tf.keras.losses.MeanSquaredError(name="loss")(2 * targets - 1, output_layer)
            else:
                raise Exception("Loss function " + params["loss_func"] + " not supported.")
            # Optimizer
    ...
    def export_graph(self, model_location, model_filename=None):

        super(GenericClassifierModel, self).export_graph(model_location, model_filename)
