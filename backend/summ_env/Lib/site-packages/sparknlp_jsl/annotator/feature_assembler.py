...
class FeaturesAssembler(AnnotatorModelInternal):
    """Collects features from different columns.

    It can collect features from single value columns (anything which can be cast to a
    float. If casts fails then the value is set to 0), array columns or SparkNLP
    annotations (if the annotation is an embedding, it takes the embedding,
    otherwise tries to cast the `result` field).

    The output of the transformer is a `FEATURE_VECTOR` annotation (the numeric vector
    is in the `embeddings` field).
    ...
    def setInputCols(self, value:str):
